    ...
    public static Optional<LinkTargetAndMethod> findLinkTarget(LinkRef linkRef)
    {
        ...
        if (matches.size() > 1) {
            throw new RuntimeException(
                "found multiple methods for linkRef: " + linkRef.getName() + " on " + linkRef.getResource().getName());
        }
        if (matches.size() == 1) {
    ...
    public static void logLinkInfo(Appendable a, LinkInfo li)
    {
        try {
            ...
            li.linkTargetAndMethodOpt.ifPresent(l -> {
                try {
                    a.append("    method:           ").append(l.method.getDeclaringClass().getName()).append("::").append(
                        l.method.getName()).append("\n");
                    a.append("    path:             ").append(l.pathOpt.map(Path::value).orElse("")).append("\n");
                    a.append("    templateParams:   ").append(
                        Arrays.stream(l.linkTarget.templateParams()).collect(Collectors.joining(", "))).append("\n");
                    a.append("    condition:        ").append(String.valueOf(l.linkTarget.condition())).append("\n");
                    a.append("    linkProc(Target): ").append(
                        Arrays.stream(l.linkTarget.linkProcessors()).map(Class::getName).collect(Collectors.joining(", "))).append("\n");
                }
                catch (IOException e) {
                    throw new RuntimeException(e);
                }
            });
            a.append("    linkProc(Ref):    ").append(
                (li.linkRef.linkProcessor != null) ? li.linkRef.linkProcessor.toString() : "(null)").append("\n");
        }
        catch (IOException e) {
            throw new RuntimeException(e);
        }
